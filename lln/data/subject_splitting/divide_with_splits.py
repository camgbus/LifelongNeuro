...
def divide_events_by_splits(events_df, site_splits, split_ix=0, ood_site_id=None):
    '''Divide a df of events into three for training, id testing and OOD testing, based on the 
    subject_id'''
    subject_division = divide_subjects(site_splits, split_ix=split_ix, ood_site_id=ood_site_id)
    events_train = events_df[events_df['subject_id'].isin(subject_division["train"])]
    events_id_test = events_df[events_df['subject_id'].isin(subject_division["id_test"])]
    events_ood_test = events_df[events_df['subject_id'].isin(subject_division["ood_test"])]
...
def divide_subjects(site_splits, split_ix=0, ood_site_id=None):
    ...
    Parameters:
        ...
        ood_site_id (str): The index of this fold. ood_site_id == site_1 means that site 1 is leaft for OOD 
            testing. The last fold is always used for ID testing (except for the site respective of 
            ood_site_id, of course, which belongs to the OOD testing set).
    Returns:
    ...
    for splits_site_id, site_folds in site_splits.items():
        ...
        for fold_id, fold_subjects in site_folds.items():
            if splits_site_id == ood_site_id:
                subject_division["ood_test"] += fold_subjects
            elif fold_id == str(split_ix):
